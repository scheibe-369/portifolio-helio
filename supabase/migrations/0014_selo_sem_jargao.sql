-- 0014_selo_sem_jargao.sql
--
-- O SELO DO PERFIL DEIXA DE NASCER DIZENDO QUE O DONO E VIBECODER.
--
-- O produto e vendido com a promessa escrita em /comprar: "Preciso saber programar? Nao".
-- Mas badge_label nascia com default 'VibeCoder' e badge_icon com 'code', e a normalizacao
-- desta funcao FORCAVA os dois de volta para quem nao pagou o bump de personalizacao. Ou
-- seja: uma advogada, uma confeiteira e um tatuador publicavam, ao lado do proprio nome, um
-- selo dizendo que sao vibecoders, e nem comprando o bump conseguiam tirar, porque ate
-- 16/08/2026 o componente de render tambem ignorava as duas colunas e imprimia o texto
-- literal.
--
-- DUAS MUDANCAS, e a segunda e de produto:
--
-- 1. As colunas perdem o default. Quem ja tem linha NAO muda: o Helio continua com
--    'VibeCoder' gravado, que e o correto para ele, e o HTML do apex sai identico.
--
-- 2. badge_label e badge_icon saem do bump e passam a ser da BASE. Eles nao sao estetica,
--    sao identidade: dizer "Chef" ou "Tatuador" ao lado do proprio nome nao pode custar
--    R$ 37,90, ainda mais quando o valor de fabrica era o nome de outra profissao. O bump
--    continua com o que e estetica de verdade: cor de destaque, fundo das placas, cor por
--    projeto, rotulo do botao principal e nome dos grupos de filtro.
--
-- O corpo de montar_payload_portfolio abaixo foi extraido do BANCO com pg_get_functiondef e
-- teve apenas as duas linhas de forcamento removidas. Nao foi redigitado: o Postgres nao
-- remenda funcao, todo create or replace troca o corpo inteiro, e esta funcao ja foi
-- reescrita por inteiro duas vezes (0004 e 0007). Redigitar e a forma mais facil de perder
-- um campo do payload de todos os compradores em silencio.


alter table myportifolio.portfolios alter column badge_label drop default;
alter table myportifolio.portfolios alter column badge_icon  drop default;

-- Camada 1 do bump: o grant por coluna. As duas saem do bloqueio e entram na base.
grant update (badge_label, badge_icon) on myportifolio.portfolios to authenticated;
