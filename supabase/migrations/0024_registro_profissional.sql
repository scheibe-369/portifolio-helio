-- 0024_registro_profissional.sql
--
-- CAMPO PARA O NUMERO DE REGISTRO NO CONSELHO: OAB, CRP, CRM, CREA, CRO, CAU, CRN, CREF.
--
-- Nao e enfeite e nao e vaidade: para varias profissoes regulamentadas o numero e OBRIGATORIO
-- em peca publicitaria, por resolucao do proprio conselho. A Resolucao CFP 010/2005 exige o
-- CRP; o Codigo de Etica da OAB exige o numero de inscricao; CFM, CREA e CAU tem regra
-- equivalente. Publicar sem ele nao e so incompleto, e infracao disciplinar de quem publicou.
--
-- O produto nao tinha onde por. As duas personas regulamentadas do teste (advogada e
-- psicologa) acabaram enfiando o registro num slot de "Numeros da capa", ao lado de metricas
-- de vaidade como "casos conduzidos" e "anos de atuacao". O dado mais formal da profissao
-- delas foi parar na vitrine de marketing porque era o unico buraco disponivel.
--
-- Fica ao lado do nome e da profissao, no card do topo, que e onde um cliente procura. Texto
-- livre e curto de proposito: cada conselho tem formato proprio ("OAB/MG 123.456",
-- "CRP 06/148372", "CREA-SP 5069...") e validar formato por sigla seria uma tabela de regras
-- que envelhece sozinha e recusa gente com registro valido.

alter table myportifolio.portfolios
  add column if not exists registro_profissional text;

alter table myportifolio.portfolios
  add constraint portfolios_registro_ok
    check (registro_profissional is null or length(registro_profissional) <= 40);

grant update (registro_profissional) on myportifolio.portfolios to authenticated;
