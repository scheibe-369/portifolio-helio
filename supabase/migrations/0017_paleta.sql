-- 0017_paleta.sql
--
-- O COMPRADOR PASSA A ESCOLHER UMA PALETA, SEM PAGAR EXTRA POR ISSO.
--
-- Ate aqui a unica cor que existia era a de fabrica: roxo #7C5CFC sobre placa #0b0b12, em
-- todo portfolio de toda profissao. Trocar isso era o item numero um do bump de R$ 37,90, e
-- mesmo pagando o comprador so mudava a cor DE CADA PROJETO, porque theme_accent e
-- theme_plate_bg nunca chegaram a ser lidos pelo render. Dez portfolios de nichos diferentes
-- foram publicados e o dono resumiu assim: ate agora nao teve nenhuma mudanca de cor, so
-- estava o template generico.
--
-- A COLUNA GUARDA O NOME DA PALETA, NUNCA O HEX. Tres consequencias, e as tres importam:
--
--   1. contraste vira teste de build. scripts/testar-presets.mjs recalcula a razao WCAG de
--      cada paleta e reprova o commit se alguem acrescentar uma ilegivel. Cor guardada como
--      dado so daria para conferir uma pagina de cada vez, depois de publicada.
--   2. as tres camadas que protegem o bump ficam INTACTAS. Quem nao comprou escreve
--      theme_preset, que e coluna nova e livre, e nunca encosta em theme_accent, que continua
--      atras do grant por coluna, do trigger portfolios_guarda_colunas e da normalizacao de
--      saida desta funcao. O trigger nem chega a disparar, porque a coluna dele nao e tocada.
--   3. a fronteira do bump fica mais clara e mais vendavel do que era:
--         base = escolher entre paletas prontas, todas validadas
--         bump = cor livre em hex por cima do preset, mais cor por projeto
--
-- O campo preset e emitido FORA do bloco de normalizacao, entao ele sobrevive para quem nao
-- tem o bump. Nulo significa a cor de fabrica, que e o estado de todo tenant ja publicado.
--
-- As colunas de fundo entram na mesma migration para nao gastar um segundo create or replace
-- desta funcao (o Postgres troca o corpo inteiro a cada vez, e ela ja foi reescrita cinco
-- vezes). O render delas vem no passo seguinte.

alter table myportifolio.portfolios
  add column if not exists theme_preset text,
  add column if not exists background_kind text not null default 'none',
  add column if not exists background_path text,
  add column if not exists background_overlay smallint not null default 55;

alter table myportifolio.portfolios
  add constraint portfolios_preset_ok
    check (theme_preset is null or theme_preset ~ '^[a-z][a-z0-9-]{1,23}$'),
  add constraint portfolios_bgkind_ok
    check (background_kind in ('none','photo')),
  add constraint portfolios_bgoverlay_ok
    check (background_overlay between 20 and 90),
  -- Mesmo confinamento de pasta que avatar_path e hero_path ja tem: o caminho precisa comecar
  -- pelo id do proprio portfolio, senao um tenant apontaria para a midia de outro.
  add constraint portfolios_bgpath_ok
    check (background_path is null or background_path like id::text || '/%'),
  -- O tipo photo exige caminho. Sem isto, o render teria que decidir o que fazer com um fundo
  -- de foto sem foto, e essa decisao nao pertence ao render.
  add constraint portfolios_bg_coerente
    check (background_kind = 'none' or (background_kind = 'photo' and background_path is not null));

-- Camada 1 das tres do bump. Nenhuma destas e coluna de seguranca e nenhuma entra no bloqueio.
grant update (theme_preset, background_kind, background_path, background_overlay)
