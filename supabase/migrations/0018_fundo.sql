-- 0018_fundo.sql
--
-- O FUNDO DA PAGINA PASSA A SER ESCOLHA, E NAO PRETO LISO PARA TODO MUNDO.
--
-- Sao sete padroes desenhados em CSS puro, mais a foto do proprio comprador. A 0017 ja tinha
-- criado as colunas e aceitava so 'none' e 'photo'; aqui o CHECK abre para os padroes e a
-- funcao passa a publicar o ramo 'background' no payload.
--
-- POR QUE CSS PURO E NAO CANVAS, que e a pergunta obvia: o render e isomorfico e roda dentro
-- do Worker, onde nao existe `document`. Fundo animado seria codigo de browser, exigiria
-- hidratacao, e a primeira pintura sairia sem fundo nenhum. Vasculhei um acervo de 799
-- efeitos prontos e dos 59 fundos de la 52 sao componentes React com WebGL, que nao portam
-- para um render que devolve string. Escrever cada um em 15 a 40 linhas de CSS saiu mais
-- barato que adaptar um, e o que se ganha e grande: renderizam no primeiro byte, funcionam
-- sem JavaScript, sao compostos na GPU e nao encostam no teto do medir-render, porque o custo
-- no servidor e concatenar uma string a mais.
--
-- O VEU sobre a foto (background_overlay) nao e estetica: e a unica coisa que garante que o
-- texto branco continue legivel sobre uma imagem que ninguem validou. Por isso o minimo de 20
-- esta no CHECK, e nao so no formulario.
--
-- 'none' continua sendo o default, entao nenhum tenant publicado muda de aparencia.

alter table myportifolio.portfolios drop constraint if exists portfolios_bgkind_ok;
alter table myportifolio.portfolios
  add constraint portfolios_bgkind_ok
    check (background_kind in ('none','photo','mesh','grid','dots','vinheta','grao','brilho'));

-- A coerencia muda junto: so o tipo 'photo' exige caminho. Os padroes de CSS nao tem arquivo.
alter table myportifolio.portfolios drop constraint if exists portfolios_bg_coerente;
alter table myportifolio.portfolios
  add constraint portfolios_bg_coerente
    check (background_kind <> 'photo' or background_path is not null);
