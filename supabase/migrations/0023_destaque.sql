-- 0023_destaque.sql
--
-- CADA TRABALHO GANHA UMA LINHA DE DESTAQUE: preco, prazo ou condicao.
--
-- Era o buraco de template mais repetido entre as dez profissoes testadas, e ele nao tinha
-- onde caber em nenhum dos 19 campos que um trabalho ja tinha:
--
--   . a confeiteira precisa de "a partir de R$ 180" e "encomende com 7 dias";
--   . o tatuador precisa de "sessao de 3h" e "orcamento pelo direct";
--   . o personal precisa de "12 semanas, acompanhamento semanal";
--   . o chef precisa de "menu de 7 tempos, R$ 220 por pessoa".
--
-- As duas foram tentadas no teste e nenhuma serve: enfiar isso na tagline gasta a frase que
-- descreve o trabalho, e enfiar em "O que esta incluso" esconde o preco atras de um clique,
-- que e o oposto do que quem vende encomenda precisa.
--
-- E UMA LINHA CURTA, e o limite de 60 e a regra de desenho: ela aparece NA GRADE, ao lado do
-- nome, onde nao existe espaco para frase. Quem quiser explicar condicao de pagamento tem a
-- descricao inteira do trabalho para isso.

alter table myportifolio.portfolio_projects
  add column if not exists highlight_i18n jsonb;

alter table myportifolio.portfolio_projects
  add constraint projects_highlight_ok
    check (myportifolio.i18n_texto_valido(highlight_i18n, 60));

grant update (highlight_i18n) on myportifolio.portfolio_projects to authenticated;
