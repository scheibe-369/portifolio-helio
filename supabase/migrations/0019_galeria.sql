-- 0019_galeria.sql
--
-- CADA TRABALHO PASSA A PODER TER MAIS DE UMA FOTO.
--
-- Ate aqui um trabalho tinha exatamente uma imagem, e ela e a miniatura do card. Isso serve a
-- um portfolio cujos trabalhos sao logos e sistemas, que foi de onde o produto nasceu, e nao
-- serve a quase nenhuma das dez profissoes testadas: um ensaio fotografico tem doze fotos, um
-- projeto de arquitetura tem planta e tres angulos, uma tatuagem tem o antes, o traco fresco e
-- o cicatrizado, um bolo tem a mesa montada e o corte. O dono pediu direto: "seria interessante
-- o cara poder botar umas fotos dentro de cada card de projeto".
--
-- A galeria e uma COLUNA jsonb, e nao uma tabela:
--
--   . e uma lista curta (teto de 8) e ordenada, editada sempre em bloco, exatamente como
--     socials e stats ja sao (0002:277-278). Tabela traria id, position, RLS e CRUD proprios
--     para reordenar tres fotos;
--   . a cota NAO depende disto. Quem conta bytes e arquivos e o trigger sobre storage.objects
--     (0003), que enxerga o arquivo no bucket independentemente de qual coluna cita o caminho.
--     Foto que entra na galeria consome cota igual, e foto removida da lista vira orfa e e
--     varrida pela faxina de 03:41 como qualquer outra.
--
-- O CONFINAMENTO DE PASTA E O QUE IMPORTA AQUI, e vale a mesma regra de avatar_path e
-- hero_path: cada caminho precisa comecar pelo id do PORTFOLIO dono do projeto. Sem isso, um
-- comprador poderia listar na galeria dele o caminho da midia de outro, que e leitura de
-- arquivo alheio num bucket publico. Como o CHECK de uma tabela filha nao enxerga o id do pai,
-- a checagem mora numa funcao que resolve o portfolio_id da propria linha.
--
-- Lista vazia continua sendo o estado de todo projeto existente: nada muda de aparencia.

create or replace function myportifolio.galeria_valida(p_galeria jsonb, p_portfolio_id uuid)
returns boolean
language plpgsql stable as $fn$
declare v_item jsonb; v_path text; v_vistos text[] := '{}';
begin
  if p_galeria is null then return true; end if;
  if jsonb_typeof(p_galeria) <> 'array' then return false; end if;
  if jsonb_array_length(p_galeria) > 8 then return false; end if;

  for v_item in select value from jsonb_array_elements(p_galeria) loop
    if jsonb_typeof(v_item) <> 'string' then return false; end if;
    v_path := v_item #>> '{}';
    if not myportifolio.media_path_valido(v_path) then return false; end if;
    -- A mesma pasta do dono, e nao qualquer pasta. Este e o ponto do CHECK.
    if v_path not like p_portfolio_id::text || '/%' then return false; end if;
    -- Repetir a mesma foto na galeria nao quebra nada, mas e sempre engano de quem clicou
    -- duas vezes, e a lista aparece na tela exatamente como esta gravada.
    if v_path = any(v_vistos) then return false; end if;
    v_vistos := v_vistos || v_path;
  end loop;
  return true;
end; $fn$;
...
alter table myportifolio.portfolio_projects
  add constraint projects_gallery_ok check (myportifolio.galeria_valida(gallery, portfolio_id));

grant update (gallery) on myportifolio.portfolio_projects to authenticated;
