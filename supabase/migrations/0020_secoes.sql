-- 0020_secoes.sql
--
-- A ORDEM DAS SECOES, E QUAIS APARECEM, PASSAM A SER DO DONO DA PAGINA.
--
-- Elas eram literais no render, na ordem stacks, projetos, experiencia, igual para todo mundo.
-- Essa ordem foi desenhada para um portfolio de programador, onde o trabalho e a vitrine e a
-- formacao e rodape, e ela e errada para metade das profissoes testadas: para uma psicologa,
-- que por sigilo profissional nao pode expor caso nenhum, a formacao E o portfolio; para um
-- professor de concursos, titulacao e aprovacao valem mais que material didatico; um tatuador
-- so quer a galeria.
--
-- ISTO REABRE, DE PROPOSITO, PARTE DA DECISAO TRAVADA N 4, que dizia em caixa alta que ordem
-- de secao nao se libera "porque isso e o LAYOUT, e o layout e o produto". A decisao foi
-- tomada olhando um portfolio so, o do dono, e caiu diante de dez. O que ela protegia continua
-- protegido: o comprador escolhe entre secoes que existem, na vertical, sem CSS proprio e sem
-- mexer em espacamento nem em tipografia. Ele nao consegue estragar o layout, so ordena o que
-- ja estava desenhado. O registro escrito da mudanca vai em tasks/_plano/decisoes-travadas.md,
-- com data e motivo, e nao apagando o texto anterior.
--
-- O PAR FOTO + PERFIL NAO ENTRA NA LISTA. Ele e a identidade da pessoa e a ancora do LCP, e o
-- editor pendura os alvos "Trocar foto" e "Editar perfil" no primeiro e no segundo filho da
-- grade do topo: se aquela grade deixasse de ser o primeiro elemento, os dois botoes sumiriam
-- do canvas sem erro nenhum.
--
-- O BANCO VALIDA A FORMA, E NAO O REGISTRO DE SECOES. Nada de CHECK com a lista literal de
-- chaves: a lista cresce a cada secao nova e viraria um ALTER TABLE por deploy. Quem descarta
-- chave desconhecida e resolverSecoes(), no render, sem lancar, porque derrubar a pagina de um
-- cliente pagante por causa de um nome de secao seria o pior desfecho possivel.

create or replace function myportifolio.secoes_validas(p jsonb) returns boolean
language plpgsql immutable as $fn$
declare v_item jsonb; v_key text; v_vistos text[] := '{}';
begin
  if p is null then return true; end if;
  if jsonb_typeof(p) <> 'array' then return false; end if;
  if jsonb_array_length(p) > 12 then return false; end if;

  for v_item in select value from jsonb_array_elements(p) loop
    if jsonb_typeof(v_item) <> 'object' then return false; end if;
    v_key := v_item ->> 'key';
    if v_key is null or v_key !~ '^[a-z][a-z0-9_]{1,31}$' then return false; end if;
    if v_key = any(v_vistos) then return false; end if;
    v_vistos := v_vistos || v_key;
    if v_item ? 'on' and jsonb_typeof(v_item -> 'on') <> 'boolean' then return false; end if;
    -- Chave estranha dentro do item e recusada: a lista e um contrato curto, e aceitar campo
    -- desconhecido aqui e como abrir espaco para gravar qualquer coisa numa coluna de conteudo.
    if exists (select 1 from jsonb_object_keys(v_item) k where k not in ('key', 'on')) then
      return false;
    end if;
  end loop;
  return true;
...
alter table myportifolio.portfolios
  add constraint portfolios_sections_ok check (myportifolio.secoes_validas(sections));

grant update (sections) on myportifolio.portfolios to authenticated;
