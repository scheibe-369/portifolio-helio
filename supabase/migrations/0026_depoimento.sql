-- 0026_depoimento.sql
--
-- DEPOIMENTO DE CLIENTE POR TRABALHO.
--
-- Foi o buraco de template mais caro que o experimento achou, e ele so apareceu quando entrou
-- uma profissao cujo produto E prova social: uma cerimonialista nao vende foto de casamento,
-- vende "a Clarice salvou o meu casamento", dito pela noiva. O produto tinha 36 campos de
-- perfil e 19 de trabalho, e nenhum deles servia.
--
-- Os quatro esconderijos foram tentados e medidos, e os quatro falham por motivos diferentes:
--   . em "o que esta incluso", a fala da noiva vira item de lista de entregaveis;
--   . em "o que voce entregou", ela se funde ao texto da propria Clarice e vira auto elogio;
--   . na frase do trabalho, e cortada em 280 caracteres, levando junto a autoria;
--   . na observacao do link, e cortada em 120 E exige inventar um link, porque casamento nao
--     tem URL.
--
-- POR TRABALHO, E NAO UMA SECAO SOLTA DE DEPOIMENTOS. Depoimento sem contexto e frase de
-- propaganda; ao lado do casamento a que ele se refere, e prova. E porque e por trabalho, ele
-- nasce junto com a foto e a data, sem uma tela nova para gerenciar.
--
-- O autor e um campo separado, e nao parte do texto, para o render poder distinguir a fala da
-- assinatura sem adivinhar por pontuacao.

alter table myportifolio.portfolio_projects
  add column if not exists depoimento_i18n jsonb,
  add column if not exists depoimento_autor text;

alter table myportifolio.portfolio_projects
  add constraint projects_depoimento_ok check (myportifolio.i18n_texto_valido(depoimento_i18n, 400)),
  add constraint projects_depoimento_autor_ok
    check (depoimento_autor is null or length(depoimento_autor) <= 80),
  -- Autor sem fala nao e depoimento, e uma assinatura solta. A fala sem autor vale (nem todo
  -- cliente autoriza o nome), entao a exigencia e so nesta direcao.
  add constraint projects_depoimento_coerente
    check (depoimento_autor is null or depoimento_i18n is not null);

grant update (depoimento_i18n, depoimento_autor) on myportifolio.portfolio_projects to authenticated;
